...
## EXAM - S3 Public Access
For S3 objects to be publically accessible you need:
...
- requester must be authenticated in AWS (no anonymous calls allowed)
...
**Purpose:** Perform bulk actions on large numbers of S3 objects (millions to billions) without writing custom scripts.

**How it works:**
- Input: S3 Inventory report or CSV file listing objects
- Action: Copy objects, invoke Lambda, change storage class, apply tags, restore from Glacier, modify ACLs, encrypt objects
- AWS handles retries, parallelism, and reporting

**Key Points:**
- Optimizes **operational effort, scalability, and reliability**
- Asynchronous; not real-time
- Costed per job + underlying S3 requests; additional charges if using Lambda, KMS, Glacier, or cross-account copy
- Reports written to S3 for completion/failure tracking
...
### S3 Object Lambda
- Lets you intercept and transform data on GET requests (and some HEAD/LIST use cases)
- uses lambda functions under the hood, to transform data
...
- **Note**: You must create an S3 Access Point first to use S3 Object Lambda.
